...
function change_settings(){
  ...
  <form method="post" action="options.php">  
    ...
    <input type="hidden" name="page_options" value="facebook_link,twitter_link,youtube_link,linkedin_link,is_related,brochure_link,delivery_link,privacy_link,tracking_sales,tracking_enquiries,tracking_emails,tracking_brochures,tracking_mobiles" />  
  </form>  
  <?php
}
...
add_filter( 'wp_nav_menu_items', 'add_loginout_link_footer', 10, 2 );
function add_loginout_link_footer( $items, $args ) {
    if (is_user_logged_in() && $args->theme_location == 'footer_menu_3') {
        $items .= '<a href="'. wp_logout_url() .'">Log Out</a>';
    }
    elseif (!is_user_logged_in() && $args->theme_location == 'footer_menu_3') {
        $items .= '<a href="'. site_url('wp-login.php') .'">Log In</a>';
    }
    return $items;
}
