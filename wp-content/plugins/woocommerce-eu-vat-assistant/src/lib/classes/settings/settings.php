...
class Settings extends \Aelia\WC\Settings {
	...
	public function default_settings($key = null, $default = null) {
		$default_options = array(
			...
			self::FIELD_EU_VAT_FIELD_TITLE => __('EU VAT Number', 'wc-aelia-eu-vat-assistant'),
			self::FIELD_EU_VAT_FIELD_DESCRIPTION => __('Enter your EU VAT Number (if any). Country prefix is not required.', 'wc-aelia-eu-vat-assistant'),
			...
			self::FIELD_SELF_CERTIFICATION_FIELD_TITLE => __('I am established, have my permanent address, or usually reside within <strong>{billing_country}</strong>.', 'wc-aelia-eu-vat-assistant'),
	...
	public function get_exchange_rates_schedule_info() {
		...
		if(wp_get_schedule($this->_exchange_rates_update_hook) === false) {
			$next_update_schedule = __('Not Scheduled', 'wc-aelia-eu-vat-assistant');
		}
		else {
		...
		if(($last_update_timestamp = $this->current_settings(self::FIELD_EXCHANGE_RATES_LAST_UPDATE)) != null) {
			...
			$last_update_timestamp_fmt = __('Never updated', 'wc-aelia-eu-vat-assistant');
		}
	...
	public function get_current_exchange_rates_provider_label() {
		...
		return get_value('label', $model_info, __('Not available', 'wc-aelia-eu-vat-assistant'));
	}
	...
	public function validate_settings($settings) {
		...
		if($this->validate_exchange_rates_provider_settings($settings) === true) {
			...
			if(empty($settings[self::FIELD_EXCHANGE_RATES]) || !empty($exchange_rates_to_update) ||
				 get_value('update_exchange_rates_button', $_POST[self::SETTINGS_KEY])) {
				...
				if($this->update_exchange_rates($settings, $errors) === false) {
					...
					add_settings_error(self::SETTINGS_KEY,
														 ...
														 __('Settings saved. Exchange rates have been updated.', 'wc-aelia-eu-vat-assistant'),
														 'updated');
				}
			}
		}
	...
	protected function register_exchange_rates_model($class_name, $label) {
		if(!class_exists($class_name) ||
			...
			throw new Exception(sprintf(__('Attempted to register class "%s" as an Exchange Rates ' .
																		 ...
																		 'Aelia\WC\IExchangeRatesModel interface.', 'wc-aelia-eu-vat-assistant'),
																	$class_name));
		}
	...
	protected function register_exchange_rates_models() {
		...
		$exchange_rates_models = apply_filters('aelia_wc_exchange_rates_models', array(
			$namespace_prefix . 'Exchange_Rates_BitPay_Model' => __('BitPay', 'wc-aelia-eu-vat-assistant'),
			$namespace_prefix . 'Exchange_Rates_ECB_Model' => __('ECB', 'wc-aelia-eu-vat-assistant'),
			$namespace_prefix . 'Exchange_Rates_HMRC_Model' => __('HMRC (UK)', 'wc-aelia-eu-vat-assistant'),
			$namespace_prefix . 'Exchange_Rates_IrishRevenueHTML_Model' => __('Irish Revenue (HTML) - WARNING: experimental, may not always work!', 'wc-aelia-eu-vat-assistant'),
			$namespace_prefix . 'Exchange_Rates_DNB_Model' => __('Danish National Bank', 'wc-aelia-eu-vat-assistant'),
		));
	...
	protected function validate_exchange_rates($exchange_rates, &$exchange_rates_to_update = array()) {
		...
		if(!empty($currency_with_invalid_rates)) {
			$this->validation_errors['invalid-rate'] = sprintf(__('Some exchange rates entered manually are invalid. ' .
																														...
																														'currencies: %s.', 'wc-aelia-eu-vat-assistant'),
