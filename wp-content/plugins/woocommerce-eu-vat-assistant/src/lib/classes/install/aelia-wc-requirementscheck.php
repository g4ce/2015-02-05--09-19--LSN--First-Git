...
if(!class_exists('Aelia_WC_RequirementsChecks')) {
	...
	class Aelia_WC_RequirementsChecks {
		...
		protected function check_required_extensions() {
			foreach($this->required_extensions as $extension) {
				if(!extension_loaded($extension)) {
					$this->requirements_errors[] = sprintf(__('Plugin requires "%s" PHP extension.', 'wc-aelia-eu-vat-assistant'),
																								 $extension);
				}
			}
		}
			...
			foreach($this->required_plugins as $plugin_name => $plugin_requirements) {
				...
				if(is_array($plugin_info)) {
					if(version_compare($plugin_info['Version'], $plugin_requirements['version'], '<')) {
						$error_message = sprintf(__('Plugin "%s" must be version "%s" or later.', 'wc-aelia-eu-vat-assistant'),
																		 $plugin_name,
					...
					$error_message = sprintf(__('Plugin "%s" must be installed and activated.', 'wc-aelia-eu-vat-assistant'),
																	 $plugin_name);
				}
		...
		public function check_requirements() {
			...
			if(PHP_VERSION < '5.3') {
				$this->requirements_errors[] = __('Plugin requires PHP 5.3 or greater.', 'wc-aelia-eu-vat-assistant');
			}
