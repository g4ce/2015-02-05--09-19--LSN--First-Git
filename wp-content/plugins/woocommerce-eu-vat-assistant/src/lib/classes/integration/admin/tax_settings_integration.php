...
class Tax_Settings_Integration extends \Aelia\WC\Base_Class {
	...
	public static function localize_admin_scripts(array $admin_scripts_params) {
		...
		$admin_scripts_params['tax_settings'] = array(
			...
			'user_interface' => array(
				'update_eu_vat_rates_button_label' => __('Update EU VAT Rates', 'wc-aelia-eu-vat-assistant'),
				'eu_vat_rates_using_text' => __('using', 'wc-aelia-eu-vat-assistant'),
				'vat_label' => __('VAT', 'wc-aelia-eu-vat-assistant'),
				'vat_updated_message' => __('VAT rates have been updated. Please review them, to ' .
																		'ensure that they are correct, then click on "Save ' .
																		'changes" to save the new rates.', 'wc-aelia-eu-vat-assistant'),
				'invalid_vat_rates' => __('An error occurred retrieving the VAT rates, therefore ' .
																	...
																	'again by reloading the page later.', 'wc-aelia-eu-vat-assistant'),
				'tax_payable_to_country' => array(
					'header_label' => __('Tax payable to Country', 'wc-aelia-eu-vat-assistant'),
					'header_tooltip' => __('Specify to which country this tax should be paid. Leave empty ' .
																 ...
																 'the one in the Country Code column).', 'wc-aelia-eu-vat-assistant'),
					'field_placeholder' => __('Auto', 'wc-aelia-eu-vat-assistant'),
				)
			),
		);
	...
	public static function woocommerce_tax_rate_added($tax_rate_id, $tax_rate_info) {
		global $wpdb;
		$country = $tax_rate_info['tax_rate_country'];
		$row_id = array_pop(explode('_', $country));
	...
	public static function woocommerce_tax_rate_updated($tax_rate_id, $tax_rate_info) {
		...
		// Update the row
		$tax_info = array(
			'tax_payable_to_country' => $_POST['tax_payable_to_country'][$tax_rate_id],
		);
