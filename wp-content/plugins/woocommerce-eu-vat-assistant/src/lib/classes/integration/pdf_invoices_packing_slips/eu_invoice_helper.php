...
class EU_Invoice_Helper {
	...
	public function target_currencies() {
		return array_unique(array($this->order_currency, $this->vat_currency));
	}
	...
	public function __construct(EU_Invoice_Order $order) {
		...
		$this->order = $order;
		$this->order_currency = $this->order->get_order_currency();
		...
		$this->vat_currency_exchange_rate = $this->EUVA()->get_order_vat_exchange_rate($order->id);
	...
	public function reverse_charge() {
		...
		$eu_vat_evidence = $this->order->eu_vat_evidence;
	...
	public function raw_woocommerce_price($price) {
		$target_currency = $this->order->get_order_currency();
	...
	public function wpo_wcpdf_billing_address($formatted_billing_address) {
		$vat_number_label = __('VAT #:', 'wc-aelia-eu-vat-assistant');
